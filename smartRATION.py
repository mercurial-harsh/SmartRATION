...
def make_transaction(supplier_key, receiver_key, item_id):
	...
	if selection == 'M':
		index = int(input('There are a total of ' + str(len(grain_intakers_list)) + ' users. Enter your selection: ')) - 1
		supplier_key = grain_intakers_list[index]
		
	elif selection == 'O':
		index = int(input('There are a total of ' + str(len(end_users_list)) + ' users. Enter your selection: ')) - 1
		supplier_key = end_users_list[index]
		
	selection = input('\nSelect type of key (M/O) for receiver: ')
	if selection == 'M':
		index = int(input('There are a total of ' + str(len(grain_intakers_list)) + ' users. Enter your selection: ')) - 1
		receiver_key = grain_intakers_list[index]
		
	elif selection == 'O':
		index = int(input('There are a total of ' + str(len(end_users_list)) + ' users. Enter your selection: ')) - 1
		receiver_key = end_users_list[index]
		...
		for index in range(0, transaction_amount):
			...
			if(verify_transaction(utxo_array[0])):
				...
				if(check_item_code(utxo_array[0])):
					...
					print('The item code was not found on blockchain. Checking for grain_intaker credentials.')
					if(check_grain_intaker_credentials(utxo_array[0])):
						print('The new item has been added under the grain_intaker.')
						transaction_array.append(utxo_array[0])
					else:
						print('The transaction key is not authorised as a grain_intaker!')
			else:
...
# Smart contract for checking if the user is an authorised grain_intaker
def check_grain_intaker_credentials(self):
	for item in grain_intakers_list:
...
# Function for generating grain_intaker keys
def generate_grain_intaker_keys(number):
	for item in range(0, int(number)):
		grain_intakers_list.append(RSA.generate(1024, Random.new().read))
	#print(grain_intakers_list)
	print('\nThe grain_intaker keys have been generated.')
	
# Function for generating stakeholder keys
def generate_other_keys(number):
	for item in range(0, int(number)):
		end_users_list.append(RSA.generate(1024, Random.new().read))
	#print(end_users_list)
	...
	for block in supply_blockchain[1:]:
		for transaction in block.supply_data:
			if(item_code == transaction.item_id):
				...
				grain_intaker_suppplier = False
				grain_intaker_receiver = False
				...
				for item in grain_intakers_list:
					...
					if str(transaction.supplier_puk.exportKey("PEM").decode('utf-8')) == str(item.publickey().exportKey("PEM").decode('utf-8')):
							...
							grain_intaker_suppplier = True
							break
				
				if(supplier_not_found_flag):
					supplier_count = 0
					
					for item in end_users_list:
						supplier_count = supplier_count + 1
				...
				if(grain_intaker_suppplier):
					final_result = final_result + "grain_intaker #" + str(supplier_count) + " transferred the asset to "
				else:
					final_result = final_result + "Stakeholder #" + str(supplier_count) + " transferred the asset to "

				if(grain_intaker_receiver):
					final_result = final_result + "grain_intaker #" + str(receiver_count) + " at " + str(transaction.timestamp)
				else:
...
number_grain_intakers = int(input('\nEnter the number of grain_intakers: '))
generate_grain_intaker_keys(number_grain_intakers)
	
number_end_users = int(input('\nEnter the number of stakeholders: '))
generate_other_keys(number_end_users)
...
while(1):
	...
	if(choice == 1):
		...
		number_grain_intakers = int(input('\nEnter the number of grain_intakers: '))
		generate_grain_intaker_keys(number_grain_intakers)
		number_end_users = int(input('Enter the number of stakeholders: '))
		generate_other_keys(number_end_users)
	elif(choice == 7):
